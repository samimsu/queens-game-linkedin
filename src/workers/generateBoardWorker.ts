...
self.onmessage = async (e) => {
  ...
  const MIN_IMPROVE_TIME_SECONDS = 100; // if we find a board but it is not the best candidate, give up after this many seconds and just use the candidate
  async function generateLevel(size: number) {
    ...
    while (Date.now() - startTime < 1000 * 60 * MAX_MINUTES_PER_LEVEL) {
      const elapsedTimeSeconds = (Date.now() - startTime) / 1000;
      const remainingTimeSeconds =
        MAX_MINUTES_PER_LEVEL * 60 - elapsedTimeSeconds;
      attemptCounter++;
