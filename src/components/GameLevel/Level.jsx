...
const Level = ({ id, level }) => {
  ...
  const [notHighlightedSquares, setNotHighlightedSquares] = useState([]); // TODO: da spostare in Board
  ...
  const handleHintButton = () => {
    ...
    if (hint.highlightedCells.length > 0) {
      const notHighlightedCells = Array(board.length * board.length)
        .fill(null).map((_, i) => {
          const row = Math.floor(i / board.length)
          const col = i % board.length
          return hint.highlightedCells.findIndex(cell => cell.row === row && cell.col === col) !== -1 ? null : { row, col }
        })
        .filter(cell => cell !== null)
        .filter(cell => hint.crossedCells.findIndex(crossedCell => crossedCell.row === cell.row && crossedCell.col === cell.col) === -1)
        setNotHighlightedSquares(notHighlightedCells)
    }
        ...
        <div>
          ...
          <div className="game relative">
            ...
            <Board
              ...
              notHighlightedSquares={notHighlightedSquares}
