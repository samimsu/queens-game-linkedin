...
const PageGeneratedLevelSizeSelection = () => {
  ...
  const [combinationsChecked, setCombinationsChecked] = useState(100);
  ...
  const createGeneratedBoard = (size: number) => {
    ...
    worker.onmessage = (e) => {
      if (e.data.type === "generatedBoard") {
        ...
        setCombinationsChecked(combinationsChecked + 100);
      }
    };
    ...
    <RootLayout>
      ...
      >
        ...
        <RefreshCw
          ...
          size={16}
        />{" "}
        <div>{combinationsChecked} boards generated</div>
      </div>
    </RootLayout>
  );
};
